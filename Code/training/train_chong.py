...
def train(net, train_dataloader, optimizer, epoch, logger):
    ...
    net.train(True)
    for i, (img, face, head_channel, gaze_heatmap, name, gaze_inside,objects_channel) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
        images = img.cuda()
        head = head_channel.cuda()
        objects =  objects_channel.cuda()
        faces = face.cuda()
        gaze_heatmap = gaze_heatmap.cuda()
        gaze_heatmap_pred, attmap, inout_pred = net(images, head, faces,objects)
